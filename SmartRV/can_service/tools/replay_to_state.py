...
def decode_candump(filename, timestamps=True, factor=1, pause=None):
    start_time = None
...
def decode_busmaster(filename, factor=1, pause=None):
    start = time.time()
    start_time = None
    # factor = 1          # >1 slower, <1 faster
    # pause = None        # Seconds or None, if <=0 wait for input
...
if __name__ == '__main__':
    ...
    if file_type == 'BUSMASTER':
        decode_busmaster(in_file)
    elif file_type == 'CANDUMP':
        decode_candump(in_file)
    else:
